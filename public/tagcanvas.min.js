...
;(function () {
	...
	function PointsOnSphere(n, xr, yr, zr, magic) {
		...
		inc =
			Math.PI *
			(3 - sqrt(5) + (parseFloat(magic) ? parseFloat(magic) : 0))
		for (i = 0; i < n; ++i) {
			y = i * off - 1 + off / 2
			r = sqrt(1 - y * y)
			phi = i * inc
	...
	Oproto.DrawSize = function (c, x, y, w, h, colour, tag, x1, y1) {
		...
		if (this.pulsate) {
			if (tag.image)
				sc =
					(tag.image.height + this.tc.outlineIncrease) /
					tag.image.height
			else sc = tag.oscale
	...
	Oproto.DrawPulsate = function (c, tag, x1, y1) {
		var diff = TimeNow() - this.ts,
			...
			ga =
				...
				(1 - t.pulsateTo) *
					(0.5 +
						cos((2 * Math.PI * diff) / (1000 * t.pulsateTime)) / 2)
	...
	Tproto.Measure = function (c, t) {
		...
		if (t.txtOpt) {
			...
			if (t.outlineMethod == 'colour') {
				...
				f =
					s * (this.textHeight + t.outlineIncrease) +
					'px ' +
					this.textFont
				c.font = f
				cw = this.MeasureText(c)
		...
		if ('colour' == m) this.colour = FindGradientColour(t, nweight, wnum)
			...
			if (t.weightSizeMin > 0 && t.weightSizeMax > t.weightSizeMin) {
				this.textHeight =
					t.weightSize *
					(t.weightSizeMin +
						(t.weightSizeMax - t.weightSizeMin) * nweight)
			} else {
				// min textHeight of 1
