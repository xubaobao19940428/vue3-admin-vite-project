<template>
    ...
    <el-sub-menu v-else :index="item.path" :key="index + '_' + item.meta.title">
        ...
        <template v-for="childRoute in item.children">
            <el-menu-item v-if="!childRoute.hidden && !childRoute.isExternal" :index="item.path + '/' + childRoute.path" :key="item.path + '/' + childRoute.path">
            ...
            <!--加上外链-->
            <el-menu-item v-if="childRoute.isExternal" :index="item.path + '/' + childRoute.path" :key="item.path + '/' + childRoute.path">
                <a :href="childRoute.path" :key="item.path + '/' + childRoute.path" target="_blank">
                    <i v-if="childRoute.meta.icon" class="iconfont" v-html="childRoute.meta.icon"></i>
                    <span>{{ childRoute.meta.title }}</span>
                </a>
